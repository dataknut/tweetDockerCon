---
title: "#Royal Wedding 2017: Tweet Analysis"
...
```{r set parameters}
startDay <- as.Date("2018-05-17") # DockerConEU 2017 - allow for pre-day excitement :-)
endDay <- as.Date("2018-05-21") # allow for post day excitement
hashTag <- "#royalwedding"
maxTweets <- 1000000
oFile <- "~/Data/twitter/royalWedding2017_tweets" # where the results are saved (do not include suffix)
...
To extract and visualise tweets and re-tweets of `#royalwedding` for 17 - 21 May, 2018.
...
# collect tweets
tweetListDT <- ba_collectTweets(maxTweets, as.Date(startDay),as.Date(endDay), hashTag, oFile)
...
tweetListDT <- tweetListDT[as.Date(obsDateTimeMins) >= as.Date(startDay, tz = timeZone) &
                             as.Date(obsDateTimeMins) <= as.Date(endDay, tz = timeZone)
                             ]
...
myCaption <- paste0("All (re)tweets containing #royalwedding ", 
...
myCaption <- paste0("All (re)tweets containing #dockercon Friday 18th May 2018")

myPlot <- ba_make5MinTimeSeriesChart(tweetListDT[obsDate == "2018-05-18"], 
...
And here's a really bad visualisation of the top 50 tweeters so we can see their tweetStreaks!
