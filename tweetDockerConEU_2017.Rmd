...
ba_setUseFullTimes <- function(dt){
  # set to central time
  dt <- dt[, createdLocal := with_tz(created, tz = "CET")] # local
...
for(d in 1:6){
  try(file <- paste0("dockerConEU2017tweetListD", d, "DT.csv"))
  try(tweetListDT <- rbind(tweetListDT,data.table(read_csv(file))))
}
...
tweetListDT <- tweetListDT[as.Date(obsDateTimeMins) >= as.Date("2017-10-15", tz = "CET") &
                             as.Date(obsDateTimeMins) <= as.Date("2017-10-19", tz = "CET")
                             ]
